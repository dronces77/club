	...
	public function convertir($id)
	{
		...
		try {
			...
			if ($prospecto->celular) {
				// Buscar el tipo de contacto "Celular" en el catálogo
				$tipoContacto = \DB::table('catalogo_tipos_contacto')->where('codigo', 'CEL')->first();
			
				if ($tipoContacto) {
					ClienteContacto::create([
						'cliente_id'       => $cliente->id,
						'tipo_contacto_id' => $tipoContacto->id, // obligatorio
